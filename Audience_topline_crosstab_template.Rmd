...
#Note that the default num_rows=7 is a standard table of the first 5 rows are regular
...
#Here is a function that can be used to update a table's css styling
update_table_styling2 = function(df, caption = "<br><br>"){
  num_rows = nrow(df)
  #where contains the position of where the styling will take effect
  #Note that where starts at index 0
  where = rbind(c(num_rows-2,1), c(num_rows-1,1), c(num_rows-1,2), c(num_rows-1,3), c(num_rows,2), c(num_rows,3), c(num_rows-1,4), c(num_rows,4), c(num_rows-1,5), c(num_rows,5))
...
for (col in matrix_q){
  table= create_toplines_with_counts_matrix(col,df)
...
display_table = function(col,df){
  ...
  if (col %in% no_subtotal_qs){
    ...
    topline_table = create_topline_cro_subtotal(temp,col,split)
  }
